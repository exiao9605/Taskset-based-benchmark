...
// ======== 可调开关 ========
#define NUM_GROUPS     1000   // 组数：每组依次跑 9 个 fragment
...
static void stats_push(Stats* st, long long x) {
...
static void stats_finish(const Stats* st, long double* mean, long double* sd) {
    if (st->n == 0) { *mean = 0; *sd = 0; return; }
    *mean = st->sum / st->n;
    long double var = 0.0L;
    if (st->n > 1) {
        var = (st->sum2 / st->n) - (*mean) * (*mean);
        if (var < 0) var = 0; // 数值误差守护
    }
    *sd = sqrt((double)var);
}
...
int main(void) {
    ...
    // 计时循环：NUM_GROUPS 组；每组依次跑 9 个 fragment
    ...
    printf("--------------------------------------------------------------------------\n");
    printf("%-35s  %-12s %-12s %-12s %-12s\n", "Fragment", "Min(ns)", "Max(ns)", "Mean(ns)", "StdDev(ns)");
    for (int i = 0; i < 9; ++i) {
        long double mean, sd;
        stats_finish(&stats[i], &mean, &sd);
        printf("%-35s  %-12lld %-12lld %-12.2Lf %-12.2Lf\n",
               fragment_names[i],
